...
use Blinkswag\Store\Services\Printful;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Mail;
use Blinkswag\Store\Models\BlinkswagStore;
use Blinkswag\Store\Models\Blinkswag_Store_printful_transparent_images;
use Illuminate\Support\Facades\Redirect;
use Illuminate\Support\Facades\Storage;
use App\Services\Inventory;
use Blinkswag\Store\Models\BlinkswagStoreUsers;
use Blinkswag\Store\Models\Blinkswag_Store_Cart_Payment;
use Blinkswag\Store\Models\Blinkswag_Store_User_ResetPassword;
use Blinkswag\Store\Models\Blinkswag_Store_Product_List;
...
class BlinkswagStoreController extends Controller
{
    ...
    public function stores(){
        ...
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_stores',["stores"=>$stores ]);
    }

    public function dashboard_stores(){
        $stores = BlinkswagStore::get();
        $Product_List = Blinkswag_Store_Product_List::get();
        //dd( $Product_List );
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_dashboard_stores',["stores"=>$stores, "Product_List"=>$Product_List ]);
    }
    ...
    public function product_editor()
    {
        ...
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_product_editor',  [
    ...
    public function create_store(Request $request){
        $store = new BlinkswagStore();
        $already_store_name = BlinkswagStore::where('name', $request->name)->get()->count();
    ...
    public function deletestore(Request $request){
        $store = BlinkswagStore::where('id', $request->id)->delete();
...
//return view('blinkswag_store::pages.viewstore',["store"=>$store, "all_categories_details"=>$all_categories_details,"products"=>$products, "my_editing_products"=>$my_editing_products, "my_editing"=>$my_editing, "googleuser"=>\Session::get('googleuser') ]);

        $store = BlinkswagStore::where("id", $id)->first();
        if($store)
        {
            $all_categories = [];
            ...
            return view('blinkswag_store::blinkswag_store_pages.blinkswag_store',["store"=>$store, "all_categories_details"=>$all_categories_details,"products_db"=>$products, "my_editing_products"=>$my_editing_products, "my_editing"=>$my_editing, "items"=>$items, "item_group"=>$item_group ]);
        }else{
    ...
    public function updatestore(Request $request){
       ...
       // dd($all_items);
        $affectedRows = BlinkswagStore::where("id", $id)->update(["complete_settings" => $complete_settings, "all_items_json"=> json_encode($all_items)]);
    ...
    public function viewstore($storename)
    {
        ...
        $store = BlinkswagStore::where(["name"=>$storename])->get()->first();
        if( $store )
        {
            $all_categories = [];
            $all_categories_details = [];
            ...
            {
                return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_viewstore',["store"=>$store, "all_categories_details"=>$all_categories_details,"products"=>$products, "my_editing_products"=>$my_editing_products, "my_editing"=>$my_editing, "googleuser"=>\Session::get('googleuser') ]);
            }else{
                return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_viewstore',["store"=>$store, "all_categories_details"=>$all_categories_details,"products"=>$products, "my_editing_products"=>$my_editing_products, "my_editing"=>$my_editing ]);
            }
        }else{
            echo 'No Store Found.';
        }
    ...
    {
        $user = BlinkswagStoreUsers::where(["email"=>$request->email, "google_id"=>0])->get();
        if( !count($user) )
        {
            $user = BlinkswagStoreUsers::Create([
                'name' => $request->name,
                'email' => $request->email,
    ...
    {
        $affectedRows = BlinkswagStoreUsers::where("id", $request->storeuserid)->update(["address" => $request->address_array]);
        $user = BlinkswagStoreUsers::find($request->storeuserid);
        \Session::put("googleuser1", $user);
        return $user;
    }
    ...
    public function store_order(Request $request)
    {
         ...
         */
        $old_SO = Blinkswag_Store_Cart_Payment::Where('stripe_payment_json', 'like', '%' . $request->payment_intent . '%')->get()->count();
        if($old_SO>0)
        {
            return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_order_success',['status' => 'already submited', 'message'=>"Salesorder is already submited.", 'store_name'=>$store_name]);
        }
        ...
        if( isset($googleuser1) && isset($store_name) && isset($cart) )
        {
            ...
            $store = BlinkswagStore::select("id", "name", "id_company")->where("name", $store_name)->get()->first();
            ...
            $Store_Cart_Payment = new Blinkswag_Store_Cart_Payment;
            ...
            return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_order_success',['status' => 'success', 'message'=>"Salesorder created and payment saved into Database.", 'store_name'=>$store_name]);
        }else{
    ...
    {
        $user = BlinkswagStoreUsers::where(["email"=> $request->email, "password"=> $request->password])->get();
        if(count($user))
        {
    ...
    public function forgetPassword(Request $request)
    {
        ...
        $user = BlinkswagStoreUsers::where(["email"=> $request->email, 'google_id'=>0])->get()->count();
        if($user == 0)
        {
            ...
            $unique_token = time().substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, 8);
            $Store_User_ResetPassword = new Blinkswag_Store_User_ResetPassword;
            $Store_User_ResetPassword->email             = $request->email;
            $Store_User_ResetPassword->token             = $unique_token;
            $Store_User_ResetPassword->save();

            $title = "Reset Password";
            Mail::to($request->email)->send( new BlinkswagStoreUserResetPasswordMail($title, $unique_token));
    ...
    {
        $token = $request->token;
        $isToken = Blinkswag_Store_User_ResetPassword::where(["token"=>$token])->count();
        if($isToken>0)
        {
            $resetpassword = Blinkswag_Store_User_ResetPassword::where(["token"=>$token])->get()->first();
            //Change Password view
            $store_user = BlinkswagStoreUsers::where(['email'=>$resetpassword['email'], 'google_id'=>0])->get()->first();
            //dd( $store_user  );
            return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_resetpassword',["store_user"=>$store_user]);
    ...
    {
        $StoreUsers = BlinkswagStoreUsers::find($request->user_id);
    ...
    public function product_printful()
    {
        ...
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_product_printful',  [
    ...
    public function getProductDetails(Request $request)
    {
        ...
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_productDetails',  [
    ...
    public function getProductDetails_popup(Request $request)
    {
        ...
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_getProductDetails_popup',  [
    ...
    {
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_getProductmockups_popup',  [
    ...
    public function getProductDetails_popup_edit(Request $request)
    {
        ...
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_getProductDetails_popup_edit',  [
            'product_details'=>$request->selected_product,
            'selected_editing'=>$request->selected_editing,
            'general_my_editing_single'=>$request->general_my_editing_single
		]);
    ...
    {
        $product_list_id = $request->product_list_id;
        $Product_List = Blinkswag_Store_Product_List::find($product_list_id);
        $stores = BlinkswagStore::get();

        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_productDetails_edit',  [
    ...
    public function getAllProductsByCategoryId(Request $request)
    {
        ...
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_printful_inner_products_view',  [
            'products'=>$products
		]);
    ...
    public function saveProductintoProductList(Request $request)
    {
        ...
        $Product_List = new Blinkswag_Store_Product_List();
    ...
    {
        $Product_List = Blinkswag_Store_Product_List::where('id', $request->id)->delete();
        return json_encode(["status"=>"success"]);
    }
    ...
    {
        $Product_List = Blinkswag_Store_Product_List::find($id);
    ...
    public function edit_printful_product($id)
    {
        ...
        $printful = new Printful();
        $product_details = $printful->getProductDetailsById($product_id);
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_productDetails_new',  [
            'product_details'=>$product_details
		]);
        //return $id;
    }
    ...
    public function addproductlist_store(Request $request)
    {
        ...
        {
            $Store = BlinkswagStore::where("name", $store)->get()->first();
    ...
    public function upload_products(){
        ...
        //$products = $printful->getAllProductsByCategory( $firstCatid );
        $products = $printful->getAllProducts();
        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_upload_products',  [
			'parent_categories' => $parent_categories,
            'products' => $products
		]);
    }
    ...
    public function view_uploaded_products()
    {
        ...
        $printful_transparent_images = Blinkswag_Store_printful_transparent_images::get()->all();

        return view('blinkswag_store::blinkswag_store_pages.blinkswag_store_view_uploaded_products',  [
            'printful_transparent_images'=>$printful_transparent_images
		]);
    }
}
