            ...
            if ($dh = opendir($dir)) {
                while (($file = readdir($dh)) !== false) {
                    if($file!="." && $file!=".." && str_contains($file, $variant_id))
                    {
                        ...
                        echo "<img style='width: 100%;height: 100%;' class='gallery_image_view img-polaroid tt' src='".url('/').'/public/Image/'.$id_company.'/mockups/'.$product_id.'/'.$file."' /> <br />";
                        echo "</div>";
                        ...
                        if($file=="front_".$variant_id.".png")
                        {
                            $flag=1;
                            ?>
                            <img style="position: absolute;right: 0px;top: 0px;width: 75%;" class="js-qv-product-cover card-img-top modal_product_image" src="{{url('/')}}/public/Image/{{$id_company}}/mockups/{{$product_id}}/{{$file}}" alt="Card image cap">
                            <?php
                        }
                    }
                }
